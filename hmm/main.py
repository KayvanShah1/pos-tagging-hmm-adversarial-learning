...
def train_and_evaluate_advs_hmm():
    # Prepare dataset
    print("\nReading and preparing data ...")
    ...
    # Initialize, train, and save the model
    print("\nTraining the HMM model ...")
    model = AdversarialHMM(vocab_file=VocabConfig.VOCAB_FILE, labels=unique_pos_tags)
    model.fit(train_sentences_with_pos_tags)
    ...
    # Save the model
    model.save_model(file_path=HMMConfig.ADVS_HMM_MODEL_SAVED)
    ...
    df_greedy_preds.to_json(PathConfig.ADVS_GREEDY_ALGO_OUTPUT_PATH, orient="records", indent=4)
    print(
        "Saved Greedy Decoding predictions to"
        f" {os.path.relpath(PathConfig.ADVS_GREEDY_ALGO_OUTPUT_PATH)}"
    )
    ...
    df_viterbi_preds.to_json(PathConfig.ADVS_VITERBI_ALGO_OUTPUT_PATH, orient="records", indent=4)
    print(
        "Saved Viterbi Decoding predictions to"
        f" {os.path.relpath(PathConfig.ADVS_VITERBI_ALGO_OUTPUT_PATH)}"
    )
